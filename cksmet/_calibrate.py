...
class Calibrator(object):
    ...
    def fit(self, catalog, node_points, suffixes=['_new','_lib'], mode="L2"):
        """
        ...
        def obj(fit_params):
            if mode=="L2":
                return np.sum(resid(fit_params)**2)
            elif mode=="L1":
                return np.sum(np.abs(resid(fit_params)))
            else:
                assert False

        out = lmfit.minimize(obj, fit_params, method='fmin')
    ...
    def hyperplane_coeff(self, x, dx, fmt='.2f'):
        ...
        Args:
             ...
             dx (dict): delta parameter used to define slope
